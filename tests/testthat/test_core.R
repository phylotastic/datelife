...
test_that("Summarize as mrca works correctly", {
  ...
  mrca.vector <- SummarizeResults(filtered.results, output.format="mrca", datelife.cache=datelife.cache)
...
test_that("Summarize as citations works correctly", {
  ...
  results.list <- lapply(datelife.cache$trees,GetSubsetArrayDispatch, taxa=taxa, phy=NULL)
  filtered.results <- ProcessResultsList(results.list, taxa, TRUE)
  citation.results <- SummarizeResults(filtered.results, output.format="cit", datelife.cache=datelife.cache)
...
test_that("Summarize as newick.all works correctly", {
  ...
  results.list <- lapply(datelife.cache$trees,GetSubsetArrayDispatch, taxa=taxa, phy=NULL)
  filtered.results <- ProcessResultsList(results.list, taxa, TRUE)
  trees <- SummarizeResults(filtered.results, output.format="newick.all", datelife.cache=datelife.cache)
...
test_that("Summarize as newick.median works correctly", {
  ...
  tree <- SummarizeResults(filtered.results, output.format="newick.median", datelife.cache=datelife.cache)
