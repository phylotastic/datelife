...
test_that("test groves", {
    grove_taxa <- filter_for_grove(names_subset2_result, criterion = "taxa")
    grove_tree <- filter_for_grove(names_subset2_result, criterion = "tree")
})
...
test_that("median and sdm give a tree when source trees have different degrees of overlapping names", {
    ...
    expect_true(inherits(summarize_datelife_result(datelife_query = names_subset2_query,
      datelife_result = names_subset2_result, summary_format = "phylo_median"), "phylo"))
    expect_true(inherits(summarize_datelife_result(datelife_query = names_subset2_query,
      datelife_result = names_subset2_result, summary_format = "phylo_sdm"), "phylo"))
    expect_equal(class(summarize_datelife_result(datelife_query = names_subset2_query,
      datelife_result = names_subset2_result, summary_format = "newick_median")), "character")
    expect_equal(class(summarize_datelife_result(datelife_query = names_subset2_query,
      datelife_result = names_subset2_result, summary_format = "newick_sdm")), "character")
})
