...
The function `make_datelife_query` processes the taxon names of your organisms of interest and saves them as a `datelifeQuery` object.

A `datelifeQuery` object is a list of three elements:
- `$cleaned_names`: a character vector of "cleaned" taxon names, 
- `$ott_ids`: a numeric vector of OpenTree Taxonomy identifiers (OTT ids), and 
- `$phy`: the input tree as a `phylo` object or `NA` if none was provided.

This allows us to straightforwardly use the processed taxon names later to perform a search for ages across the `datelife` chronogram database and other downstream analyses. 
