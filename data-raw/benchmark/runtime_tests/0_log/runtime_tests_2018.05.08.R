...
res$tnrs <- rep("True", 2000)
res2$tnrs <- rep("False", 2000)
...
plt <- plt + ggplot2::stat_ydensity(ggplot2::aes(color = tnrs, fill = tnrs))  # this and follow line work the same
...
plt <- plt + ggplot2::scale_color_manual(values = c("grey", "black")) +
...
plt
pdf(file="make_datelife_query_runtime_BOTH_NoLegend_xtime_all_labels.pdf", width = 6, height = 3)
...
pdf(file="make_datelife_query_runtime_BOTH_NoLegend_xtime_some_labels1.pdf", width = 6, height = 3)
...
plt
dev.off()
pdf(file="make_datelife_query_runtime_BOTH_WithLegend_xtime_some_labels1.pdf", width = 6, height = 3)
plt <- plt + ggplot2::theme(legend.position = c(0.07,0.87), legend.title = ggplot2::element_text(size = 5, face = "bold"), 
