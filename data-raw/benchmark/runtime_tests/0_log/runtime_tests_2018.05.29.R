...
# Runtime tests plots, query plus get_datelife_result runtimes
...
# 3. get_datelife_results:
res3 <- c()
ninput <- c(10,100,200,300,400,500,600,700,800,900, 1000,2000,3000,4000, 5000, 6000,7000,8000,9000,10000)
for(i in ninput){
  x <- paste0("gfr_runtime_2018.04.07_",
              i, "_aves_spp")
  xname <- paste0(x, ".RData")
  load(xname)
  res3 <- rbind(res3, get(x))
}
ls()
length(res3)
str(res3)
str(res3$expr)
ggplot2::autoplot(res3)
res3$Time <- microbenchmark:::convert_to_unit(res3$time, "t")  #changing the name of the element itself is the easiest way to make it appear as axis label

#########
res$tnrs <- rep("Using TNRS", 2000)
res2$tnrs <- rep("Not using TNRS", 2000)
res3$tnrs <- rep("DateLife search", 2000)
res_all <- dplyr::bind_rows(res, res2, res3)
names(res_all)
...
gbot_runtime_2018.03.26_50009_aves_spp
...
gbot_runtime_2018.03.26_5000_10to100_aves_spp$time[2:92]
...
pdf(file= paste0("~/Google Drive/datelife/runtime_tests/2_tests/3_gbot/",
                 "gbot_2018.05.29_1.pdf"), width = 6, height = 3)
plt <- ggplot2::ggplot(res4, ggplot2::aes_string(x = "expr", y = "Time"))
plt <- plt + ggplot2::coord_cartesian(ylim = c(y_min, y_max))
# plt <- plt + ggplot2::stat_ydensity()
# # this plots both at the same time:
plt <- plt + ggplot2::stat_ydensity(scale = "width", width = 0.5)
# color controls the outer line color; fill control the inside color.
