...
input_check <- function(input, ...){
	badformat <- TRUE
	if(is.list(input) & "phy" %in% names(input) & "cleaned_names" %in% names(input)) badformat <- FALSE
	if(badformat){
...
datelife_query_length_check <- function(cleaned_names, get_spp_from_taxon, verbose = FALSE){
