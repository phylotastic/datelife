...
#' @return Nothing
#' @export
run<-function(input=c("Rhea americana", "Pterocnemia pennata", "Struthio camelus"), format="html", partial="yes", plot.width=600, plot.height=600, usetnrs="no", approximatematch="yes", datelife.cache=datelife.cache) {
  ...
  FastRWeb::out(SummarizeResults(filtered.results, output.format=format, partial, datelife.cache, suppress.citations=TRUE)  )
}