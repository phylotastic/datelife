...
use_calibrations_pathd8 <- function(phy, calibrations, expand = 0.1, giveup = 100){
    ...
    if(expand != 0) {
        attempts = 0
        original.calibrations <- calibrations
        try(chronogram <- geiger::PATHd8.phylo(phy, calibrations), silent = TRUE)
        # original.calibrations <- get_all_calibrations(phy2)
        while(is.null(chronogram) & attempts < giveup) {
            # print(rep(attempts, 10))
            calibrations <- original.calibrations
            calibrations$MaxAge <- calibrations$MaxAge * ((1+expand)^attempts)
            calibrations$MinAge <- calibrations$MinAge * ((1-expand)^attempts)
            ...
            calibrations[dim(calibrations)[1]+1,]<- c("fixed", 0, 0, phy$tip.label[1], "tinytip", "none")
            chronogram <- tryCatch(geiger::PATHd8.phylo(phy2, calibrations), error = function(e) NULL)
            # data that errors in geiger::PATHd8.phylo(phy2, calibrations) (and hence the tryCatch):
            # phy2 <- cetaceae_phyloall[2]# cetaceae_phyloall[3] and cetaceae_phyloall[4] also error
            # calibrations <- get_all_calibrations(cetaceae_phyloall[2])
        ...
        calibrations <- calibs$calibrations
    }
    if(!is.null(chronogram)) {
