...
match_all_calibrations <- function(phy, calibrations) {
  ...
  target_tree_nodes <- sapply(seq(nrow(calibrations)),
                              function(i)
                              phytools::findMRCA(tree = phy, type = "node",
  ...
  phy <- tree_add_nodelabels(tree = phy,
                             node_index = node_index)
  phy$calibration_distribution <- stats::setNames(all_ages,
                                                  all_nodes_numbers)
  return(list(phy = phy,
